...
#include <iosfwd>
  ...
  public:
     ...
     */
    __device__ void zero(double val = 0.0f){
      entries[0] =
      entries[1] =
     ...
     */
    __device__ static Matrix3x3 identity( void ){
      Matrix3x3 B;

    B(0,0) = 1.f; B(0,1) = 0.f; B(0,2) = 0.f;
    B(1,0) = 0.f; B(1,1) = 1.f; B(1,2) = 0.f;
     ...
     */
    __device__ Vector3D& column( int i ){return entries[i];}
    __device__ const Vector3D& column( int i ) const{return entries[i];}

    /**
     * Returns the transpose of A.
     */
    __device__ Matrix3x3 T( void ) const{
    ...
    __device__ Vector3D& operator[]( int i ){return entries[i];}
    __device__ const Vector3D& operator[]( int i ) const{return entries[i];}

    // increments by B
    __device__ void operator+=( const Matrix3x3& B ){
        Matrix3x3& A( *this );
        double* Aij = (double*) &A;
    ...
    // returns -A
    __device__ Matrix3x3 operator-( void ) const{
      // returns -A (Negation).
        const Matrix3x3& A( *this );
        Matrix3x3 B;
    ...
    // returns A-B
    __device__ Matrix3x3 operator-( const Matrix3x3& B ) const{
        const Matrix3x3& A( *this );
        Matrix3x3 C;
    ...
    // returns c*A
    __device__ Matrix3x3 operator*( double c ) const{
        const Matrix3x3& A( *this );
        Matrix3x3 B;
    ...
    // returns A*x
    __device__ Vector3D operator*( const Vector3D& x ) const{
...
// returns the outer product of u and v.
  __device__ Matrix3x3 outer( const Vector3D& u, const Vector3D& v ){
    Matrix3x3 B;
...
// returns c*A
  __device__ Matrix3x3 operator*( double c, const Matrix3x3& A ){
    Matrix3x3 cA;
